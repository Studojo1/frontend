...
export function ResumePreview({ doc }: { doc: ResumeDoc }) {
  ...
  return (
    <div className="bg-white border-2 border-neutral-900 rounded-[24px] shadow-[6px_6px_0px_0px_rgba(25,26,35,1)] overflow-hidden h-full flex flex-col">
      {/* Header bar */}
      <div className="flex-shrink-0 bg-neutral-900 text-white px-4 py-2 text-xs font-bold uppercase tracking-wider font-['Satoshi'] flex items-center justify-between">
      ...
      <div className="flex-1 overflow-y-auto bg-neutral-100 p-4">
        {!hasAny ? (
          <div className="flex h-full items-center justify-center py-20">
            <p className="text-center text-neutral-400 font-['Satoshi'] text-sm">
              Your resume will appear here as you chat.
            </p>
          </div>
        ) : (
          /* A4-styled inner paper */
          <div className="max-w-[210mm] mx-auto bg-white shadow-[0_2px_16px_rgba(0,0,0,0.08)] px-8 py-8">
            {c.full_name && (
                  ...
                  <div key={i} className="mb-3">
                    ...
                    {e.bullets.length > 0 && (
                      <ul className="list-disc pl-5 mt-1 space-y-0.5">
                        {e.bullets.map((b, j) => (
                          <li key={j} className="text-sm text-neutral-800">{b.text}</li>
                        ))}
                      </ul>
                    )}
                  </div>
