  ...
  return (
    <div className="bg-white border-2 border-neutral-900 rounded-2xl p-4 shadow-[4px_4px_0px_0px_rgba(25,26,35,1)] font-['Satoshi']">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className={`text-2xl font-black ${barColor.replace("bg-", "text-").replace("-400", "-600").replace("-500", "-600")}`}>
          {score} <span className="font-normal text-neutral-500 text-sm">/ 100</span>
        </span>
      </div>
      ...
      <div className={`text-xs font-semibold mt-1.5 ${score >= 80 ? "text-emerald-600" : score >= 60 ? "text-violet-600" : score >= 40 ? "text-amber-600" : "text-neutral-500"}`}>{tier}</div>
